...
def create_user(user: User) -> None:
    ...
    session = next(session_gen)
    try:
        from services.core.models_db import UserDB
...
def get_user_by_oauth(provider: str, provider_id: str) -> Optional[User]:
    ...
    session = next(session_gen)
    try:
        from services.core.models_db import OAuthAccountDB, UserDB
...
def save_password(user_id: str, password_hash: str) -> None:
    ...
    session = next(session_gen)
    try:
        from services.core.models_db import PasswordDB
...
def link_oauth(user_id: str, provider: str, provider_id: str) -> None:
    ...
    session = next(session_gen)
    try:
        from services.core.models_db import OAuthAccountDB
...
def link_space_to_user(user_id: str, space_id: str) -> None:
    ...
    session = next(session_gen)
    try:
        from services.core.models_db import UserSpaceDB
