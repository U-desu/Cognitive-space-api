...
- Fallback debate templates when LLM fails

When migrating to real LLM, this file becomes a fallback/cache layer.
When migrating to Redis, these presets can be loaded as cache seeds.
"""
...
# ── Fallback debate for when LLM fails ──
FALLBACK_DEBATES = {
    "default": {
        "transcript": [
            {
                "round": 1,
                "turns": [
                    {"agent": "agent_001", "type": "argument", "content": "我方认为当前是最佳时机，窗口期有限，应该果断行动。", "evidence": []},
                    {"agent": "agent_002", "type": "rebuttal", "content": "但风险过高，盲目入场失败率极高，应谨慎行事。", "evidence": []},
                ]
            },
            {
                "round": 2,
                "turns": [
                    {"agent": "agent_001", "type": "argument", "content": "我们可以先用副业验证PMF，降低试错成本，而不是直接all in。", "evidence": []},
                    {"agent": "agent_002", "type": "rebuttal", "content": "副业和全职创业的心态完全不同，无法真实验证市场需求。", "evidence": []},
                ]
            },
        ],
        "synthesis": {
